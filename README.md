## Hi, I'm Juhan 👋

Mathematics & Quantitative Finance student at NUS (graduating 2027).
Competing in the WorldQuant International Quant Championship (IQC).
Interested in systematic trading, portfolio risk, and quantitative research.

**Current projects:**
- Monte Carlo VaR/CVaR engine with GBM simulation
- Multi-asset portfolio optimizer using MPT and scipy SLSQP
- Singapore equity risk analysis (STI constituents)

📍 Singapore  |  🎓 NUS  |  📊 WorldQuant IQC
